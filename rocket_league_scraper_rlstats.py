...
def get_html(platform,name):
    # get the HTML for filete filete from rlstats.net
    response = requests.get("https://rlstats.net/profile/"+platform+"/"+name)
    playFile = open(r"C:\Users\felix\Downloads\rlstats.html","w", encoding="utf-8")
...
def get_stats(platform, name):
    ...
    rlstats_html = get_html(platform,name)
...
time = 60 * 45 * 1 # sec * min * hour
step = 60 *  3 * 1 # total time and period step length
